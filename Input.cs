...
namespace PG2
{
    public static class Input //Since we're in a static class we need input.Method to call our methods
    {
        public static int ReadInteger(string prompt, int min, int max)
        {
            ...
            while (true)
            {
                ...
                if (!ParseNum1 || num1 < min || num1 > max)
                {
                    
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("Please input a number from the menu only!");
                    Console.ResetColor();
                    Console.WriteLine("Press any key to continue");
                    Console.ReadKey();
                    
                }
                if(ParseNum1 == true && num1 >= min && num1 <= max)
                {
        ...
        public static void ReadString(string prompt, ref string Rstring)
        {
            while (true)
            {
                ...
                if(string.IsNullOrWhiteSpace(Rstring))
                {
                    Console.WriteLine("Please enter a string, Steve!");
        ...
        public static void ReadChoice(string prompt, string [] options, out int selection)
        {
            ...
            for (int i = 0; i < options.Length; i++)
            {
                ...
                selection = Input.ReadInteger(prompt, 1, options.Length);
                
            
            //Use a for loop
            //Use readinteger method from earlier
            //Max is set to Array.Length
        }
